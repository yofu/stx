...
func (stw *Window) fig2keyword(lis []string, un bool) error {
	...
	case "focus":
		...
		default:
			...
			for i, _ := range []string{"FOCUSX", "FOCUSY", "FOCUSZ"} {
				if lis[1+i] == "_" {
					continue
				}
	...
	case "angle":
		...
		for i, _ := range []string{"PHI", "THETA"} {
			if lis[1+i] == "_" {
				continue
			}
			val, err := strconv.ParseFloat(lis[1+i], 64)
	...
	case "dists":
		...
		for i, _ := range []string{"DISTR", "DISTL"} {
			if lis[1+i] == "_" {
				continue
			}
			val, err := strconv.ParseFloat(lis[1+i], 64)
	...
	case "unit":
		if usage {
			...
			stw.History(fmt.Sprintf("CURRENT FORCE UNIT: %s %.3f", stw.Frame.Show.UnitName[0], stw.Frame.Show.Unit[0]))
			stw.History(fmt.Sprintf("CURRENT LENGTH UNIT: %s %.3f", stw.Frame.Show.UnitName[1], stw.Frame.Show.Unit[1]))
			showhtml("UNIT.html")
			return nil
		}
		if un {
	...
	case "position":
		...
		xpos, err := strconv.ParseInt(lis[2], 10, 64)
		if err != nil {
			return err
		}
		ypos, err := strconv.ParseInt(lis[3], 10, 64)
		if err != nil {
			return err
		}
		switch strings.ToUpper(lis[1]) {
		case "PAGETITLE":
			stw.PageTitle.Position[0] = int(xpos)
			stw.PageTitle.Position[1] = int(ypos)
		case "TITLE":
			stw.Title.Position[0] = int(xpos)
			stw.Title.Position[1] = int(ypos)
		case "TEXT":
			stw.Text.Position[0] = int(xpos)
			stw.Text.Position[1] = int(ypos)
		case "LEGEND":
